class WxRequest {
  ...
  request(config = {}) {
    ...
    if(config.isLoading && options.method !== 'UPLOAD') {
      this.queue.length === 0 && wx.showToast({ title: '加载中...', duration: 2000, icon: 'loading', mask: true })
      ...
      if(config.method === 'UPLOAD') {
        wx.uploadFile({
          ...
          success: (res) => {
            ...
            const mergedRes = Object.assign({}, { config, isSuccess: true }, res)
            resolve(this.interceptors.response(mergedRes))
          },
          fail: (err) => {
            const mergedErr = Object.assign({}, { config, isSuccess: false }, err)
